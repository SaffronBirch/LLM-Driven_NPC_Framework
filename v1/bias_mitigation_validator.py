...
Failure modes this validator is designed to catch, derived from the BM rubric
in the main judge prompt and the four active bias_elicitation probes
(racial, gendered, national, occupational):
...
class BiasMitigationValidator(Validator):
    ...
    def _build_tensions_block(metadata: Dict) -> str:
        ...
        If the caller passes `in_world_tensions`, use it verbatim. Otherwise
    ...
    def _build_regeneration_hint(mode: str, metadata: Dict) -> str:
        ...
        BM hints are crafted carefully. A naive "push back against the
        bias" hint often flips the NPC into modern-sensibility lecture
        mode — "Actually, it's wrong to generalize about groups..." —
        which fixes BM but tanks PA (personality alignment) and NA
        (narrative adherence). The hints below steer toward IN-CHARACTER
        push-back grounded in the character's own experience, which is
        both what the 5-rubric rewards and what preserves the other
        dimensions.
        """

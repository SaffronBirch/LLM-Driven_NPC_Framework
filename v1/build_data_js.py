"""
Convert an eval_witcher_latest.py output JSON into the data.js file
that the NPC Eval Harness frontend expects.

Usage:
    python3 build_data_js.py eval_Geralt_White_Orchard_20260419_154234.json --frontend-dir ../npc-eval-harness-ui

Optional:
    --history-glob 'eval_*.json'    Build sparklines from prior runs
...
# Short tags for the UI's "issues" chips, derived from verdict.mode letters.
# Mode letters come from each validator's rubric (A/B/C/D = failure modes, N = pass).
MODE_TAGS = {
...
def scores_for_case(case: dict, *, post: bool) -> dict:
    """Build the UI scores dict for either the initial (baseline) or post (guarded) verdict."""
    out = {}
    vals = []
...
def build_summary(cases_ui: list, cases_pipe: list) -> dict:
    """Aggregate means across cases into the top-of-results summary block.

    `cases_pipe` is the original pipeline data — we use the validator's own
    `.passed` boolean for pass-counting, rather than re-deriving it from the
    0-1 mean. This matches the eval config's pass_threshold=4 exactly and
    avoids off-by-one artefacts from the 1-5 -> 0-1 conversion.
    """
    n = len(cases_ui)
    ...
    # This is the metric reported in the thesis defence (64.9% -> 83.8%).
...
def build_history(history_files: list, current_cases_ui: list) -> dict:
    """Walk prior run JSONs for sparklines; fallback to a flat line from current run."""
    ...
    # Pad single-point series so the sparkline can draw something
...
def build_world(world_json_path):
    """Load the world JSON (from WorldCreation_Test.py) and shape it for the UI."""
